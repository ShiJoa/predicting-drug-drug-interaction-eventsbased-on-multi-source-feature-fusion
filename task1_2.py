...
def prepare(df_drug, feature_list, mechanism, action, drugA, drugB):
    ...
    new_label = torch.tensor(new_label,dtype=torch.long)  # 323539
...
def feature_vector(feature_name, df):
    ...
    # 构造所有药物的特征，len(drug_list)行，len(all_feature)列
...
class BERT(torch.nn.Module):
    def __init__(self, input_dim, n_heads, n_layers, event_num):
        ...
        self.l2 = torch.nn.Linear((len_after_AE * 5 + event_num) // 2 + 496, event_num)

        self.ac = gelu

        self.CNN_lays = nn.Sequential(
        ...
        self.l3 = torch.nn.Linear(496, event_num)
    ...
    def forward(self, X):
        ...
        X6 = self.CNN_lays(torch.unsqueeze(torch.stack((X1,X2,X3,X4,X5), dim=1), dim=1))
        ...
        # X = self.l2(X)
        X = self.l2(torch.cat((X, torch.squeeze(torch.squeeze(X6, dim=1), dim=1)), 1))
        # X = self.l3(torch.squeeze(torch.squeeze(X,dim=1),dim=1))
...
def cross_val(feature,label,event_num):
    ...
    for train_index, test_index in skf.split(feature, label):
        ...
        pred_score=BERT_train(model,X_train,y_train,
                              X_test,y_test,event_num)
