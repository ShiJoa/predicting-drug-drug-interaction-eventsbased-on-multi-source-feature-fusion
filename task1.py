...
class BERT(torch.nn.Module):
    ...
    def forward(self, X):
        ...
        '''
        X_5=X1+X2+X3+X4
        X_5=torch.unsqueeze(X_5,dim=0)
        X_5=self.mycov(X_5)
        #print("it seems OK!")
        #os.system("pause")
        
        self.mycov = Mycov(len(16))
...
class Mycov(torch.nn.Module):
    def __init__(self, vector_size):
        ...
        self.co2_1 = torch.nn.Conv1d(256, 50, kernel_size=10)
        self.co1_1 = torch.nn.Conv1d(50, 5, kernel_size=5)
        self.pool1 = torch.nn.AdaptiveAvgPool1d(len_after_AE)
        #self.conv2_drop=torch.nn.Dropout1d()
...
def BERT_train(model, x_train, y_train, x_test, y_test, event_num):
    ...
    model = torch.nn.DataParallel(model)    #使用nn.DataParallel函数来用多个GPU来加速训练。
    ...
    datagather_loss_train=[]
    datagather_loss_test=[]
    ...
    for epoch in range(epo_num):
        ...
        for batch_idx, data in enumerate(train_loader, 0):
            ...
            loss = lam * my_loss(X, targets_a, inputs, X_AE1, X_AE2, X_AE4) + \
            (1 - lam) * my_loss(X, targets_b, inputs,X_AE1, X_AE2, X_AE4)
        ...
        datagather_loss_train.append(running_loss / len_train)
        datagather_loss_test.append(testing_loss / len_test)

    painting(datagather_loss_train,datagather_loss_test,mydata)
...
def painting(datagather_loss_train,datagather_loss_test,mydata):
    loss_train=datagather_loss_train
    loss_test=datagather_loss_test

    loss, = plt.plot(loss_train, label='loss')
    testing_loss, = plt.plot(loss_test, label='testing_loss')

    plt.legend(handles=[loss, testing_loss], loc='upper left')
    # 设置标题及字体大小
...
def main():
    ...
    painting(silver_medal,bronze_medal,mydata)
